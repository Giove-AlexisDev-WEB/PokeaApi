...
document.addEventListener("DOMContentLoaded", function () {
  _axios.default.get('https://pokeapi.co/api/v2/pokemon').then(function (response) {
    var pokemons = response.data.results;
    console.log(pokemons);
    var rowElement = document.querySelector('#pokemonList');
    pokemons.forEach(function (pokemon) {
      _axios.default.get(pokemon.url).then(function (response) {
        var pokemonData = response.data;

        _axios.default.get("https://pokeapi.co/api/v2/pokemon-species/".concat(pokemonData.id)).then(function (response) {
          var speciesData = response.data;
          var frenchData = speciesData.names.filter(function (el) {
            return el.language.name == 'fr';
          });
          var frenchName = frenchData[0].name; // console.log(speciesData)
